...
def select_name_and_series_subgenres_of_authors
  "SELECT authors.name, subgenres.name FROM authors join series
   ON authors.id == series.author_id JOIN subgenres ON series.subgenre_id==subgenres.id;"
end

def select_series_title_with_most_human_characters
  "SELECT series.title FROM series JOIN books ON
  series.id==books.series_id JOIN character_books ON books.id==character_books.book_id
